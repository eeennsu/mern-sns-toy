...
const Form: FC = () => {
    ...
    const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
        ...
        const { creator, message, selectedFile, tags, title } = formData;
        
        if (!creator || !message || !selectedFile || !tags || !title) {
        ...
        if (selectedPost) {
            ...
            const createPostDispatch = createPost(formData);
    ...
    return (
        <aside className='relative flex flex-col justify-center h-full p-4 mt-8 transition duration-500 bg-white rounded-sm shadow-2xl md:mt-0 ites-center hover:-translate-y-3'>
            ...
            <form className='flex flex-col gap-4 mt-2' autoComplete='off' noValidate onSubmit={handleSubmit}>
                <div>
                    <input className='w-full px-3 py-2 text-base text-black border rounded-sm outline-none bg-inherit focus:border-black' name='creator' placeholder='Creator' value={formData.creator} onChange={handleInputChange} />
                </div>  
                <div>
                ...
                <div>
                    <textarea className='w-full px-3 py-2 text-base text-black border rounded-sm outline-none bg-inherit focus:border-black' name='message' placeholder='Message' value={formData.message} onChange={handleInputChange} />
                </div>
                ...
                <div>
                    <FileBase type='file' multiple={false} onDone={({ base64 }: any) => setFormData(prev => ({ ...prev, selectedFile: base64 }))}>
